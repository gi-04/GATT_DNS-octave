...
%                eta = eta + mesh.attractorRegions(i,5)*interp1(nodePositions,[0 0 1 1 0 0],xBase,'pchip').^2;
                eta = eta + mesh.attractorRegions(i,5)*pchip(nodePositions,[0 0 1 1 0 0],xBase).^2;
%                eta(find(isnan(eta))) = 0; % parece que esta solução não é adequada
%                disp(eta)

          end
%          fid2 = fopen('out2.txt','w');
%          for i = 1:length(eta)
%            fprintf(fid2,"%f ",eta(i))
%          end
%          fclose(fid2);
%          pause
        end
...
%        disp(length(XPhysical))
%        XPhysical(find(isnan(XPhysical))) = 0;
%        disp(XPhysical(1))
...
%		XPhysical = XPhysical - interp1(closestNodes,XPhysical(closestNodes)-fixPoints,1:mesh.n,'pchip');
    XPhysical = XPhysical - pchip(closestNodes,XPhysical(closestNodes)-fixPoints,1:mesh.n);
%    XPhysical(find(isnan(XPhysical))) = 0;
	else
...
%    Xnew = interp1(linspace(0,1,length(X)),X,linspace(0,1,nx),'pchip');
    Xnew = pchip(linspace(0,1,length(X)),X,linspace(0,1,nx));
%    Xnew(find(isnan(Xnew))) = 0;
    Xnew(1:(1+er):nx) = X;
    X = Xnew;
end
